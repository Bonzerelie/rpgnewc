// Updated script.js for C Major Game
...
const cMajorNoteOrder = ['C', 'D', 'E', 'F', 'G', 'A', 'B'];
...
function buildNoteButtons() {
  ...
  const keys = cMajorNoteOrder.slice(0, currentMode);
  ...
  keys.forEach(note => {
    ...
    btn.textContent = showDegrees ? (currentMode === 8 && note === 'C' ? '1st/8th' : degreeMap[note]) : note;
    if (currentMode === 8 && note === 'C' && showDegrees) {
      btn.classList.add('wide-label');
    }
...
function handleAnswer(e) {
  ...
  if (selected === correctName) {
    ...
    promptText.textContent = showDegrees
      ? `Correct! ✅ The note was the ${currentMode === 8 && correctName === 'C' ? '1st/8th' : degreeMap[correctName]} scale degree`
    ...
    promptText.textContent = showDegrees
      ? `Incorrect! ❌ The note was the ${currentMode === 8 && correctName === 'C' ? '1st/8th' : degreeMap[correctName]} scale degree`
...
function toggleDisplay(mode) {
  ...
  scaleLabel.textContent = 'Diatonic notes of the C Major Scale (Ionian Mode)';
  ...
  playRefBtn.textContent = 'Play Reference, C (Tonic)';
  promptText.textContent = 'Which note was played?';
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (playScaleBtn) playScaleBtn.addEventListener('click', () => playNote('cmajorscale'));
